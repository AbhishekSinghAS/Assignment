...
exports.login = async function (req, res) {
  ...
  await User.findOne({ username: username }, async function (err, foundUser) {
    if (err) {
      console.log("Error occurred in finding user");
    } else {
      if (foundUser) {
        if (password === foundUser.password) {
          ...
          res.render("login", {
            condition: true,
            message:
              "Invalid Username/Password, If you haven't Signed up please Signup first",
          });
        }
      } else {
      ...
      if (err) {
        ...
        if (foundUser.batch1 === 0) {
          res.render("home", {
            ...
            batch1: foundUser.batch1,
            batch2: foundUser.batch2,
        ...
        if (err) {
          ...
          if (age >= 18 && age <= 65) {
            ...
            res.render("join", {
              condition: 2,
              message:
                "Person from age 18 till 65 are eligible for this Yoga class",
            });
          }
        }
      }
...
exports.batch_select = async function (req, res) {
  ...
  if (batch1 === "1") {
    if (one > 0) {
          ...
          if (err) {
            ...
            if (foundUser.batch1 === 0) {
              ...
              res.render("batch", {
                condition: true,
                message:
                  "Batch for this current month cannot be changed, you can change the batch for next month",
                slot1: one,
                slot2: two,
                slot3: three,
                slot4: four,
      ...
      res.render("batch", {
        condition: true,
        message: "Slots for this batch are not available",
        slot1: one,
        slot2: two,
        slot3: three,
        slot4: four,
    ...
    res.render("batch", {
      condition: true,
      message: "Enter batch between 1 and 4 inclusive",
      slot1: one,
      slot2: two,
      slot3: three,
      slot4: four,
